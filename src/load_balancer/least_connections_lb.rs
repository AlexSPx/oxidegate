...
impl LeastConnectionsStrategy {
    pub fn new(servers: Vec<String>) -> Self {
        log::info!("LeastConnectionsStrategy initialized with servers: {:?}", servers);
        Self {
...
impl LoadBalancer for LeastConnectionsStrategy {
    async fn next(&self) -> Option<Arc<SelectedLB>> {
        let (server, connections) = self.servers.iter()
            .min_by_key(|(_, connections)| connections.load(std::sync::atomic::Ordering::Relaxed))?;

        log::debug!("LeastConnectionsStrategy selected server: {}, current connections: {}", server, connections.load(std::sync::atomic::Ordering::Relaxed));
